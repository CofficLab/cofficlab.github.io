...
const handleSizeChange = (event: CustomEvent) => {
    selectedSize.value = event.detail;
    isLoadedFromStorage.value = true;
};
...
onMounted(() => {
    ...
    window.addEventListener('bannerBoxSizeChange', handleSizeChange as EventListener);
...
onUnmounted(() => {
    ...
    window.removeEventListener('bannerBoxSizeChange', handleSizeChange as EventListener);
});
</script>